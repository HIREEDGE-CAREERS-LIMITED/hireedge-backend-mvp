...
/**
 * Full EDGEX pipeline: detect → resolve → orchestrate → recommend → plan → compose.
 *
 * @param {string} message   - User's raw message
 * @param {object} [context] - Incoming session context from the frontend
 * @returns {{ ok: boolean, data: { reply, intent, insights, recommendations, next_actions, context } }}
 */
export function composeChatResponse(message, context = {}) {
  ...
  // 3. Update context with newly extracted entities
  const updatedCtx = updateContext(ctx, detected.entities, detected.intent, _summariseTurn(detected));

  // 4. Check whether we have enough data to run engines
  const readiness = checkReadiness(updatedCtx, detected.intent);

  // 5. If not ready, return a targeted clarification instead of generic fallback
  if (!readiness.ready) {
  ...
  // 6. Orchestrate — calls the correct intelligence engine(s) for this intent
  ...
  // 8. Plan next actions (the clickable buttons shown under the reply)
  const nextActions = planNextActions(detected.intent, insights, updatedCtx);

  // 9. Compose conversational reply from structured engine output
...
function _composeReply(intent, insights, recommendations, ctx) {
  switch (intent) {
    case "transition":   return _composeTransition(insights, ctx);
    case "explore":      return _composeExplore(insights, ctx);
    case "skills_gap":   return _composeSkillsGap(insights, ctx);
    case "salary":       return _composeSalary(insights, ctx);
    case "interview":    return _composeInterview(insights, ctx);
    case "resume":       return _composeResume(insights, ctx);
    case "linkedin":     return _composeLinkedin(insights, ctx);
    case "visa":         return _composeVisa(insights, ctx);
    case "career_pack":  return _composeCareerPack(insights, ctx);
    case "role_info":    return _composeRoleInfo(insights, ctx);
    case "compare":      return _composeCompare(insights, ctx);
    default:             return _composeGeneral(insights, ctx);
  }
}
...
function _composeTransition(ins, ctx) {
  ...
  if (roadmap?.reachable) {
    parts.push(
      `I've mapped a path from ${_title(ctx.role)} to ${_title(ctx.target)}. ` +
      `It's a ${roadmap.summary.total_steps}-step journey estimated at around ` +
      `${roadmap.summary.total_estimated_years} years.`
    );
  } else if (roadmap?.reachable === false) {
    parts.push(
      `There's no standard career path from ${_title(ctx.role)} to ${_title(ctx.target)} ` +
      `in our dataset. This would be an unconventional move — but that doesn't mean it's impossible.`
    );
  }
  ...
  if (gap?.verdict) {
    const verdictLabels = {
      ...
      moderate:    "a moderate challenge",
      hard:        "a significant challenge",
      very_hard:   "a very demanding transition",
      unreachable: "an unconventional path",
    };
    parts.push(
      `This transition is ${verdictLabels[gap.verdict] || gap.verdict} ` +
      `(difficulty score: ${gap.composite_score}/100).`
    );
  }
  ...
  if (skillsGap?.analysis) {
    ...
    parts.push(
      `Skills readiness: ${readiness_pct}% — you have ${matched_count} of the required skills ` +
      `and need ${missing_count} more.`
    );
  }
  ...
  return (
    parts.join(" ") ||
    `I can help you plan the move from ${_title(ctx.role)} to ${_title(ctx.target)}. Let me pull up the details.`
  );
}
...
function _composeExplore(ins, ctx) {
  ...
  if (moves.length > 0) {
    parts.push(
      `As a ${_title(ctx.role)}, you have ${moves.length} career progression ` +
      `option${moves.length !== 1 ? "s" : ""}.`
    );
  ...
  if (salary?.category_benchmark) {
    parts.push(
      `Your current role sits at the ${salary.category_benchmark.percentile_in_category}th ` +
      `percentile in ${salary.category_benchmark.category} salaries.`
    );
  }

  return parts.join(" ") || `Let me explore career options for ${_title(ctx.role)}.`;
}
...
function _composeSkillsGap(ins, ctx) {
  ...
  if (!gap) {
    return "I need your skills list and a target role to analyse your skills gap. " +
           "Try: \"I know SQL, Python, and Excel — what skills do I need for data engineer?\"";
  }
  ...
  if (matched.length > 0) {
    parts.push(
      `You already have: ${matched.slice(0, 5).join(", ")}` +
      `${matched.length > 5 ? ` and ${matched.length - 5} more` : ""}.`
    );
  }
  if (missing.length > 0) {
    parts.push(
      `You need to learn: ${missing.slice(0, 5).join(", ")}` +
...
function _composeSalary(ins, ctx) {
  ...
  for (const [key, val] of Object.entries(ins)) {
    if (key.startsWith("salary_") && key !== "salary_comparison" && val?.salary) {
      parts.push(
        `${val.title}: £${val.salary.mean?.toLocaleString()} ` +
        `(range: £${val.salary.min?.toLocaleString()}–£${val.salary.max?.toLocaleString()}).`
      );
    }
  }
...
function _composeInterview(ins, ctx) {
  ...
  parts.push(
    `I've prepared ${prep.competency_questions.length} competency questions, ` +
    `${prep.technical_questions.length} technical questions, and ` +
    `${prep.behavioural_questions.length} behavioural questions with STAR framework guidance.`
  );

  if (prep.salary_negotiation) {
    parts.push(
      `Salary negotiation range: ` +
      `£${prep.salary_negotiation.target_range.min.toLocaleString()}–` +
      `£${prep.salary_negotiation.target_range.max.toLocaleString()}.`
    );
  }
...
function _composeResume(ins, ctx) {
  ...
  return (
    `Your ATS compatibility score for ${blueprint.target_role.title} is ${ats.score}/100 (${ats.label}). ` +
    `I've generated a full resume blueprint with keyword strategy, skills prioritisation, section ordering, ` +
    `and bullet templates. ${blueprint.keywords.matched.length} of your skills are keyword matches, and ` +
    `${blueprint.keywords.missing_critical.length} critical keyword` +
    `${blueprint.keywords.missing_critical.length !== 1 ? "s are" : " is"} missing.`
  );
}
...
function _composeLinkedin(ins, ctx) {
  ...
  return (
    `Your LinkedIn profile strength score is ${linkedin.strength_score.score}/100 (${linkedin.strength_score.label}). ` +
    `I've generated ${linkedin.headlines.length} headline variants, a structured About section blueprint, ` +
    `and a skills prioritisation strategy. ` +
    `${linkedin.skills_strategy.top_3.length > 0 ? `Pin these as your top 3: ${linkedin.skills_strategy.top_3.join(", ")}.` : ""}`
  );
}
...
function _composeCareerPack(ins, ctx) {
  ...
  if (!pack?.ok) {
    return "I need your current role, target role, and skills to build a full Career Pack.";
  }

  const summary = pack.summary;
  return (
    `Your Career Pack is ready. Overall readiness: ${summary.overall_readiness}%. ` +
    `The pack includes a ${summary.roadmap.total_steps}-step roadmap (~${summary.roadmap.estimated_years} years), ` +
    `skills gap analysis (${summary.skills.readiness_pct}% ready), ` +
    `ATS-optimised resume blueprint (score: ${summary.resume.ats_score}/100), ` +
    `interview prep, LinkedIn optimisation, ` +
    `salary intelligence (${summary.salary.growth_pct > 0 ? "+" : ""}${summary.salary.growth_pct}% salary growth), ` +
    `and UK visa assessment.`
  );
}
...
function _composeRoleInfo(ins, ctx) {
  ...
  if (salary?.salary) {
    parts.push(
      `UK salary: £${salary.salary.mean.toLocaleString()} ` +
      `(range: £${salary.salary.min.toLocaleString()}–£${salary.salary.max.toLocaleString()}).`
    );
  }

  parts.push(
    `Core skills: ${(profile.skills_grouped?.core || []).join(", ") || profile.skills.slice(0, 5).join(", ")}.`
  );
  parts.push(
    `${profile.career_mobility.next_roles_count} progression path` +
    `${profile.career_mobility.next_roles_count !== 1 ? "s" : ""} available.`
  );
...
function _composeCompare(ins, ctx) {
  ...
  if (comp.salary_comparison.difference_pct !== null) {
    parts.push(
      `Salary difference: ${comp.salary_comparison.difference_pct > 0 ? "+" : ""}` +
      `${comp.salary_comparison.difference_pct}% (£${Math.abs(comp.salary_comparison.difference || 0).toLocaleString()}).`
    );
  }
...
function _composeGeneral(ins, ctx) {
  // If a role was found in the message, surface its profile
  if (ins.role_profile) {
    return (
      `I found information about ${ins.role_profile.title}. ` +
      `What would you like to know — career paths, salary, skills requirements, or something else?`
    );
  }
  if (ins.mentioned_role) {
    return (
      `I found information about ${ins.mentioned_role.title}. ` +
      `What would you like to know — career paths, salary, skills requirements, or something else?`
    );
  }

  // CHANGED: "HireEdge Career Copilot" → "EDGEX"
  return (
    "Hi! I'm EDGEX, your AI career intelligence engine. " +
    "Tell me your current role, target role, and skills — I'll help you with career paths, " +
    "skills gaps, resume optimisation, interview prep, salary insights, and more."
  );
}
...
function _clarificationPrompt(field) {
  switch (field) {
    ...
    case "a role to check salary for":    return "Tell me about [role name]";
